...
Mô hình đề xuất CDDFuse-AG (Asymmetric Gating) giữ nguyên kiến trúc CDDFuse, chỉ thay thế phép cộng ở Fusion Layer bằng hai quy tắc bất đối xứng:
...
So với CDDFuse baseline ($1{,}188{,}272$ tham số), CDDFuse-AG chỉ thêm 4.161 tham số ($\approx 0{,}35\%$) từ WAvg ($\theta$) và SML refine Conv. Toàn bộ tham số được huấn luyện end-to-end; Encoder dùng chung trọng số (shared weights) cho cả hai modality đầu vào.
...
\begin{itemize}\itemsep2pt
    ...
    \item Kiểm soát số tham số: toàn bộ đóng góp mới chỉ là 4.161 tham số, cho phép kết luận rõ ràng rằng cải thiện đến từ thiết kế quy tắc, không phải từ tăng dung lượng mô hình.
\end{itemize}
...
Phép cộng Sum của CDDFuse gốc áp dụng cùng một chiến lược cho cả hai nhánh, không khai thác được sự khác biệt này. Đề xuất CDDFuse-AG áp dụng hai quy tắc khác nhau cho hai nhánh, theo thiết kế bất đối xứng (\emph{asymmetric}).
...
\begin{itemize}\itemsep0pt
    ...
    \item Không có tham số học: quy tắc hoàn toàn dựa trên đặc trưng đầu vào, không phụ thuộc vào khởi tạo hay dữ liệu huấn luyện, đảm bảo tính tổng quát hóa cao.
\end{itemize}
