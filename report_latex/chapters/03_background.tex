...
Ảnh kỹ thuật số được biểu diễn dưới dạng ma trận số nguyên hai chiều (ảnh xám) hoặc ba chiều (ảnh màu). Mỗi phần tử của ma trận --- gọi là \emph{điểm ảnh} (pixel) --- lưu giá trị cường độ ánh sáng trong khoảng $[0, 255]$ với định dạng 8-bit, hoặc $[0, 1]$ sau khi chuẩn hóa. Trong bài toán tổng hợp ảnh, hầu hết các phương pháp xử lý ảnh xám (thang độ xám, một kênh) vì mỗi modality y tế về cơ bản là bản đồ cường độ đơn kênh.

Tuy nhiên, đối với ảnh PET và SPECT, thông tin chức năng thường được trực quan hóa bằng bản đồ màu giả (pseudo-color) nhằm giúp bác sĩ phân biệt các vùng hoạt động chuyển hóa. Trong trường hợp này, mô hình cần xử lý ảnh màu mà vẫn bảo toàn thông tin cường độ gốc. Đây là lý do không gian màu \textbf{YCbCr} được ứng dụng rộng rãi trong xử lý ảnh y tế tổng hợp.

\paragraph{Không gian màu YCbCr.}
YCbCr là không gian màu tách biệt thành phần \emph{độ chói} (luminance) $Y$ và hai thành phần \emph{độ màu sắc} (chrominance) $C_b$, $C_r$:
\begin{equation}
...
Bốn phương thức ảnh y tế chính được sử dụng trong đồ án có đặc điểm vật lý và thông tin bổ sung cho nhau, được tóm tắt trong Bảng~\ref{tab:modalities}.
...
&
\vspace{4pt}\includegraphics[width=0.14\textwidth]{figures/modality_samples/SPECT.png}\vspace{2pt}
&
...
% ----------------------------------------------------------------
\subsection{Tập dữ liệu Harvard Medical Image Fusion}
\label{ssec:harvard}

Đồ án sử dụng tập dữ liệu \textbf{Harvard Medical Image Fusion}~\cite{zhao2023cddfuse}, bộ dữ liệu chuẩn được sử dụng rộng rãi trong cộng đồng nghiên cứu MIF. Tập dữ liệu gồm các cặp ảnh não người thật, được thu thập và công bố bởi Wellesley College Brain Imaging Center.
...
Đồ án dùng phân chia:
\begin{itemize}\itemsep2pt
    \item \textbf{Tập huấn luyện}: 738 cặp ảnh (246 mỗi modality CT/PET/SPECT).
    \item \textbf{Tập kiểm thử}: 72 cặp ảnh (24 mỗi modality), được giữ cố định cho tất cả thực nghiệm.
\end{itemize}

Kích thước ảnh gốc thay đổi từ $182\times218$ đến $256\times256$ pixel. Hình~\ref{fig:harvard-grid} minh họa sự đa dạng về cấu trúc não và chất lượng ảnh giữa các bệnh nhân trong tập kiểm thử.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{figures/fig_harvard_grid.png}
    \caption{Mẫu ảnh từ tập kiểm thử Harvard: 5 bệnh nhân × 4 modality (CT, MRI, PET, SPECT).}
    \label{fig:harvard-grid}
\end{figure} Tất cả ảnh đều đã được đăng ký (registered) --- tức là hai ảnh trong mỗi cặp đã được căn chỉnh không gian, đảm bảo từng pixel trong ảnh CT/PET/SPECT tương ứng chính xác với cùng vị trí giải phẫu trong ảnh MRI.
...
Mạng nơ-ron tích chập là nền tảng của xử lý ảnh hiện đại. Phép tích chập 2D áp dụng bộ lọc (kernel) $k \in \mathbb{R}^{K \times K}$ trượt qua ảnh đầu vào để trích xuất đặc trưng cục bộ:
\begin{equation}
...
Transformer ban đầu được thiết kế cho xử lý ngôn ngữ tự nhiên, nhưng đã được chuyển đổi thành công sang xử lý ảnh bởi Dosovitskiy et al.~\cite{dosovitskiy2020vit}. Ý tưởng cốt lõi là \emph{coi mỗi patch nhỏ của ảnh như một token từ trong câu}, từ đó áp dụng Transformer encoder không thay đổi.
...
\end{equation}
trong đó $\mathbf{E} \in \mathbb{R}^{d \times (P^2 C)}$ là ma trận chiếu học được, $N = HW/P^2$ là số patch, và $\mathbf{p}_i \in \mathbb{R}^d$ là \emph{positional embedding} --- vector học được mã hóa vị trí không gian của patch thứ $i$ (vì self-attention không có khái niệm thứ tự vốn có).

\textbf{Bước 2 --- Transformer Encoder Block.}
...
\begin{itemize}\itemsep1pt
    ...
    \item $\mathrm{MSA}$: Multi-head Self-Attention --- mỗi token đồng thời làm query \emph{và} key/value cho tất cả token còn lại, cho phép học phụ thuộc tầm xa toàn cục $O(N^2)$.
...
Vì MSA tính tương tác giữa \emph{mọi cặp} token, chi phí tăng bậc hai theo số token:
\begin{equation}
...
\textbf{Multi-Dconv Head Transposed Attention (MDTA).}
Thay vì tính attention trên \emph{chiều không gian} (mỗi pixel là một token), MDTA tính attention trên \emph{chiều kênh}. Cụ thể, với đặc trưng đầu vào $\mathbf{X} \in \mathbb{R}^{C \times H \times W}$:
...
\begin{tabular}{lcc}
    ...
    \hline\\[-6pt]
    \multicolumn{3}{c}{\normalsize\textbf{MDTA nhanh hơn $\approx$1000 lần} so với Spatial Attention}
\end{tabular}
\end{tcolorbox}

\textbf{Gated-Dconv Feed-Forward Network (GDFN).}
...
\label{eq:gdfn}
\end{equation}
trong đó $\phi$ là GELU, $\odot$ là nhân theo phần tử (element-wise), và $W_1, W_2$ là tích chập $1\!\times\!1$. Cơ chế gating cho phép mạng kiểm soát \emph{chủ động} luồng đặc trưng nào được truyền tiếp --- nhánh trái học ``nên giữ thông tin gì'', nhánh phải học ``thông tin nào cần lọc bỏ''.

\textbf{Cấu trúc một Restormer Block.}
...
Mạng CNN thông thường thực hiện ánh xạ một chiều $f: \mathbf{x} \to \mathbf{y}$. Một khi đặc trưng $\mathbf{y}$ đã được tính, \textbf{không thể khôi phục lại $\mathbf{x}$ gốc} --- thông tin đã bị nén và mất đi không thể đảo ngược. Với ảnh y tế, điều này có nghĩa là các chi tiết biên mỏng (ví dụ ranh giới khối u trong MRI) có thể bị lọc mất qua các lớp convolution.

\paragraph{Ý tưởng cốt lõi của INN.}
Mạng nơ-ron khả nghịch (Invertible Neural Network --- INN) xây dựng ánh xạ $f$ sao cho luôn tồn tại $f^{-1}$ tính được hiệu quả:
\[
\mathbf{x} \;\xrightarrow{f}\; \mathbf{y} \;\xrightarrow{f^{-1}}\; \mathbf{x}
\]
\textbf{Không có thông tin nào bị mất}: toàn bộ nội dung của $\mathbf{x}$ được mã hóa trong $\mathbf{y}$ và có thể phục hồi hoàn toàn. CDDFuse sử dụng INN cho nhánh \emph{Detail CNN Encoder} để đảm bảo thông tin modality-specific (tần số cao) không bị mất khi mã hóa.

\paragraph{Affine Coupling Layer (RealNVP).}
Câu hỏi thực tế là: \emph{làm thế nào để xây dựng được $f$ khả nghịch mà không cần $f$ phải có dạng đặc biệt?} RealNVP~\cite{dinh2017density} trả lời bằng thủ thuật \textbf{chia đôi và coupling}:

\textbf{Bước 1 --- Chia đôi đầu vào.}
...
\textbf{Bước 2 --- Biến đổi có điều kiện (chiều thuận).}
Nửa $\mathbf{x}_1$ được biến đổi \emph{có điều kiện trên} $\mathbf{x}_2$, và ngược lại:
\begin{align}
...
\end{align}
trong đó $s_i(\cdot)$ (scale) và $t_i(\cdot)$ (translation) là các mạng CNN con tùy ý --- \emph{không cần phải khả nghịch}. Phép nhân $\odot \exp(s)$ là phép nhân theo phần tử, có tác dụng co giãn từng kênh đặc trưng.

\textbf{Bước 3 --- Nghịch đảo (chiều ngược).}
...
\begin{figure}[H]
    ...
    \caption{Cấu trúc một khối INN (Affine Coupling Layer): chiều thuận (trái sang phải) và chiều nghịch (phải sang trái) đều tính được hiệu quả.}
    \label{fig:inn-block}
\end{figure}
...
\textbf{Tại sao khả nghịch?} Dù $s_i, t_i$ là mạng CNN bất kỳ, phép biến đổi toàn khối vẫn khả nghịch vì:
\begin{itemize}\itemsep1pt
    \item Phương trình~\eqref{eq:inn-fwd1} biến đổi $\mathbf{x}_1$ nhưng \emph{giữ nguyên} $\mathbf{x}_2$ $\Rightarrow$ $\mathbf{x}_2$ vẫn đọc được từ $\mathbf{y}_2$ (bước đầu khi đảo ngược).
    ...
    \item \textbf{Tính khả nghịch đến từ cấu trúc coupling}, không phải từ $s_i, t_i$ --- do đó có thể dùng bất kỳ mạng nào cho $s_i, t_i$ mà không lo mất invertibility.
\end{itemize}
\end{tcolorbox}

\paragraph{Ứng dụng trong CDDFuse.}
Nhánh \emph{Detail CNN Encoder} (DCE) gồm 3 khối coupling xếp nối tiếp. Mỗi ảnh đầu vào $I \in \mathbb{R}^{1 \times H \times W}$ được mở rộng lên 64 kênh qua tích chập rồi đi qua 3 lớp INN liên tiếp để thu đặc trưng detail $f^D \in \mathbb{R}^{64 \times H \times W}$. Tính khả nghịch đảm bảo \textbf{mọi chi tiết tần số cao} (biên mỏng, texture) đều được giữ nguyên trong $f^D$ --- điều mà CNN thông thường không đảm bảo được.

% ================================================================
\section{Mô hình CDDFuse}
\label{sec:cddfuse}

CDDFuse~\cite{zhao2023cddfuse} (CVPR 2023) là phương pháp tổng hợp ảnh đa phương thức dựa trên ý tưởng \emph{phân rã đặc trưng theo tương quan} (Correlation-Driven Feature Decomposition). Thay vì coi toàn bộ đặc trưng ảnh như một khối đồng nhất, CDDFuse phân rã chúng thành hai thành phần có bản chất khác nhau: \emph{Base} (thông tin chung, tần số thấp) và \emph{Detail} (thông tin đặc trưng từng modality, tần số cao), rồi thực hiện fusion riêng cho từng thành phần.

% ----------------------------------------------------------------
\subsection{Kiến trúc tổng quan}
\label{ssec:cddfuse-arch}

Mô hình gồm ba thành phần chính theo sơ đồ \textbf{Encoder $\to$ Fuse $\to$ Decoder} như minh họa trong Hình~\ref{fig:cddfuse-arch}.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/cddfuse_pipeline.png}
    \caption{Kiến trúc tổng quan CDDFuse: hai ảnh đầu vào đi qua Encoder dùng chung trọng số, phân rã thành đặc trưng Base ($f^B$) và Detail ($f^D$), được tổng hợp riêng qua Fusion Layer, rồi giải mã thành ảnh đầu ra $I_F$.}
...
\begin{itemize}\itemsep1pt
    \item \textbf{Overlap Patch Embedding}: tích chập $3\!\times\!3$ chuyển ảnh 1 kênh thành đặc trưng 64 kênh, giữ nguyên kích thước $H \times W$.
    \item \textbf{Shallow Feature Extraction (SFE)}: 4 Restormer block xếp nối tiếp, học đặc trưng tầm xa qua MDTA.
    \item \textbf{Base Transformer Encoder (BTE)}: một Restormer block với spatial multi-head attention, sinh đặc trưng $f^B \in \mathbb{R}^{64 \times H \times W}$.
    \item \textbf{Detail CNN Encoder (DCE)}: 3 khối INN (RealNVP), sinh đặc trưng $f^D \in \mathbb{R}^{64 \times H \times W}$.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/encoder.png}
    \caption{Chi tiết kiến trúc Encoder CDDFuse: từ ảnh đầu vào qua Patch Embedding, SFE, rồi tách thành nhánh Base (BTE) và nhánh Detail (DCE).}
...
\end{align}
Phép cộng $f_I + f_V$ trước khi qua layer là quy tắc tổng hợp đối xứng mặc định của CDDFuse --- đây chính là thành phần mà đồ án này cải tiến.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/fusion.png}
    \caption{Chi tiết Fusion Layer: Base và Detail của hai modality được tổng hợp riêng biệt qua BaseFuseLayer và DetailFuseLayer.}
...
\begin{figure}[H]
    ...
    \caption{Chi tiết kiến trúc Decoder CDDFuse: concat đặc trưng Base và Detail đã fusion, tái tạo ảnh đầu ra $I_F$ qua Restormer blocks và output head.}
...
CDDFuse áp dụng \emph{curriculum learning} với hai pha huấn luyện tách biệt:
...
\begin{itemize}\itemsep1pt
    ...
    \item $L_{\mathrm{decomp}} = \dfrac{(\mathrm{CC}_D)^2}{1.01 + \mathrm{CC}_B}$: \emph{Correlation-Driven loss} cốt lõi. Tử số ép correlation của Detail hai modality về 0 (Detail cần mang thông tin riêng biệt); mẫu số thưởng khi Base hai modality tương quan cao (Base cần mang thông tin chung).
...
Để xác định vị trí của CDDFuse và lý do chọn làm base model, đồ án đánh giá 22 phương pháp trên cùng tập test Harvard 72 cặp ảnh với 22 chỉ số chất lượng, xếp hạng bằng Composite Z-score (chi tiết tại Mục~\ref{sec:sota-zscore}).
...
CDDFuse đứng \textbf{vị trí thứ 2} với $\bar{z} = 0{,}926$. Lý do chọn CDDFuse thay vì MM-Net-Fusion (rank 1): khoảng cách composite z chỉ $0{,}10$, trong khi CDDFuse có kiến trúc mô-đun hóa rõ ràng, tham số nhẹ ($1{,}2$M), mã nguồn và checkpoint được công bố đầy đủ, và có \emph{điểm cải tiến cụ thể} tại Fusion Layer --- đây là điều kiện lý tưởng để nghiên cứu tác động của fusion rule trong khuôn khổ đồ án.
...
Đồ án sử dụng \textbf{22 chỉ số chất lượng} theo chuẩn VIFB~\cite{liu2020vifb}, chia thành sáu nhóm theo bản chất đo lường. Phần này trình bày định nghĩa toán học chi tiết cho các chỉ số được sử dụng trực tiếp trong đánh giá và so sánh. Ký hiệu chung: $A$, $B$ là hai ảnh đầu vào (ví dụ MRI và CT), $F$ là ảnh tổng hợp đầu ra, kích thước $M \times N$.
...
Đo độ tương đồng cấu trúc giữa ảnh fused và ảnh tham chiếu dựa trên luminance, contrast và structure:
\begin{equation}
...
Đo mức độ hoạt động tần số không gian tổng thể của ảnh fused:
\begin{equation}
...
Đo độ sắc nét trung bình của ảnh fused thông qua gradient cục bộ:
\begin{equation}
...
Đo cường độ biên tổng thể sử dụng bộ lọc Laplacian:
\begin{equation}
