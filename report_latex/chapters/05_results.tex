...
\centering
\small
\caption{Giá trị trung bình theo modality (sau khi fix bug PSNR/RMSE và NaN inference). $\uparrow$ cao tốt hơn, $\downarrow$ thấp tốt hơn. \textbf{In đậm} = tốt hơn.}
...
\midrule
SSIM $\uparrow$   & \textbf{1.4434} & 1.4417          & \textbf{1.3235} & 1.3188          & 1.4031          & \textbf{1.4035} \\
PSNR $\uparrow$   & 14.40           & \textbf{14.50}  & \textbf{17.53}  & 17.06           & 21.40           & \textbf{21.40}  \\
QG   $\uparrow$   & 0.6183          & \textbf{0.6196} & \textbf{0.7638} & 0.7608          & 0.7503          & \textbf{0.7504} \\
QM   $\uparrow$   & 0.0046          & \textbf{0.0050} & \textbf{0.1230} & 0.1115          & \textbf{0.0788} & 0.0777          \\
NABF $\downarrow$ & 0.0323          & \textbf{0.0296} & 0.0153          & \textbf{0.0143} & \textbf{0.0194} & 0.0198          \\
QSF  $\uparrow$   & \textbf{0.0488} & 0.0377          & 0.0546          & \textbf{0.0557} & \textbf{0.0460} & 0.0440          \\
QMI  $\uparrow$   & 0.7830          & \textbf{0.7994} & \textbf{0.8020} & 0.7942          & 0.9049          & \textbf{0.9099} \\
MI   $\uparrow$   & \textbf{53.68}  & 51.53           & \textbf{68.33}  & 67.36           & \textbf{40.50}  & 40.49           \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Phần trăm cải thiện của CDDFuse-AG so với CDDFuse-Paper-MIF (đã hiệu chỉnh hướng cho metric ``thấp tốt hơn''). \textbf{In đậm xanh} = cải thiện $> +3\%$; \textit{nghiêng đỏ} = regression $< -3\%$.}
...
PSNR   & $+0.69\%$          & $-2.71\%$         & $+0.02\%$ \\
QG     & $+0.22\%$          & $-0.39\%$         & $+0.01\%$ \\
QM     & \textbf{+8.31\%}   & \textit{$-9.34\%$}& $-1.46\%$ \\
NABF ($\downarrow$) & \textbf{+8.31\%} (đỡ artifact) & \textbf{+6.84\%} (đỡ artifact) & $-2.07\%$ (nhỉnh xấu) \\
QSF    & \textit{$-22.65\%$}& $+2.03\%$         & \textit{$-4.27\%$} \\
QMI    & $+2.10\%$          & $-0.97\%$         & $+0.56\%$ \\
MI     & \textit{$-4.01\%$} & $-1.42\%$         & $-0.02\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}\itemsep0pt
    \item \textbf{CT}: cải thiện rõ nhất ở \emph{NABF $+8.3\%$} (giảm artifact biên) và \emph{QM $+8.3\%$} (chất lượng wavelet). Đánh đổi rõ ở QSF ($-22.7\%$) và MI ($-4\%$).
    \item \textbf{PET}: NABF cải thiện $+6.8\%$, QSF nhỉnh hơn $+2\%$. Tuy nhiên QM giảm $-9.3\%$ và PSNR giảm $-2.7\%$.
    \item \textbf{SPECT}: gần như tương đương baseline ở mọi metric (chênh lệch $< 2\%$), trừ QSF giảm $-4.3\%$.
\end{itemize}
...
\centering
\small
\caption{Trung bình trên 72 cặp test gộp 3 modality (sau fix bug). \textbf{In đậm} = tốt hơn.}
\label{tab:cddfuse-ag-pooled}
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{Baseline} & \textbf{CDDFuse-AG} & \textbf{Cải thiện (\%)} \\
\midrule
SSIM $\uparrow$   & \textbf{1.3900} & 1.3880          & $-0.15\%$ ($\approx$ tie) \\
PSNR $\uparrow$   & \textbf{17.78}  & 17.65           & $-0.70\%$ ($\approx$ tie) \\
QG   $\uparrow$   & \textbf{0.7108} & 0.7103          & $-0.07\%$ ($\approx$ tie) \\
QM   $\uparrow$   & \textbf{0.0688} & 0.0647          & $-5.94\%$ \\
NABF $\downarrow$ & 0.0224          & \textbf{0.0212} & \textbf{$+4.97\%$} (đỡ artifact) \\
QSF  $\uparrow$   & \textbf{0.0498} & 0.0458          & $-7.97\%$ \\
QMI  $\uparrow$   & 0.8300          & \textbf{0.8345} & $+0.55\%$ \\
MI   $\uparrow$   & \textbf{54.17}  & 53.13           & $-1.92\%$ \\
EN   $\uparrow$   & \textbf{5.04}   & 5.03            & $-0.21\%$ ($\approx$ tie) \\
VAR  $\uparrow$   & \textbf{67.28}  & 66.15           & $-1.68\%$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Đánh giá tổng quan.}
Trên 72 cặp test gộp:
\begin{itemize}\itemsep0pt
    \item \textbf{NABF giảm $4.97\%$}: ít ảo ảnh biên hơn --- đây là mục tiêu chính của Adaptive Gating, cho thấy hiệu quả thực tế.
    \item \textbf{QMI tăng $0.55\%$}: feature-level mutual information nhỉnh hơn.
    \item Các metric khác hầu hết \emph{tương đương} baseline ($|$chênh lệch$| < 1\%$) hoặc giảm nhẹ ($2\text{--}8\%$ ở QSF, QM, MI, VAR).
\end{itemize}
Đây là biểu hiện của ``smoothing trade-off'' --- model làm sạch biên (NABF) nhưng đánh đổi nhẹ thông tin nguồn (MI, VAR). Chênh lệch nhỏ ở SSIM/PSNR/QG cho thấy AG không phá hỏng chất lượng cấu trúc cơ bản.
...
\midrule
Số SIG / 22 metric & \textbf{9} & 2 & 4 & 2 \\
Verdict           & CONFIRM    & MIXED      & MIXED & MIXED \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Diễn giải.}
\begin{itemize}\itemsep0pt
    \item \textbf{CT đạt CONFIRM\_IMPROVEMENT} (9 metric SIG $\ge 5$ ngưỡng): CDDFuse-AG \emph{cải thiện có ý nghĩa thống kê} đối với CT, đặc biệt ở NABF, QM, QMI và các metric edge-related.
    \item \textbf{PET chỉ 2 SIG}: CDDFuse-AG không cải thiện đáng kể PET so với baseline retrain (vì baseline retrain đã đạt PET tốt sẵn). Top improvement là NABF (giảm artifact) nhưng không vượt ngưỡng Holm.
    \item \textbf{SPECT 4 SIG marginal}: hiệu quả khiêm tốn, một vài chỉ số nhỏ đạt SIG.
    \item \textbf{Pooled 2 SIG} (NABF + FMI): gain ở CT bị ``loãng'' khi pool với PET/SPECT $\to$ verdict tổng quan MIXED.
\end{itemize}
