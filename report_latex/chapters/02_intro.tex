...
\begin{itemize}\itemsep2pt
    \item \textbf{Thành phần cơ sở} (Base) mang thông tin cấu trúc chung, tương quan cao giữa hai modality --- hai ảnh thường \emph{đồng thuận} ở mức độ tổng thể. Với thành phần này, trung bình hóa là hợp lý.
    \item \textbf{Thành phần chi tiết} (Detail) mang các cạnh, texture và thông tin đặc thù của từng modality --- hai ảnh mang thông tin \emph{bổ sung nhau}, không phải lặp lại nhau. Với thành phần này, cần \emph{lựa chọn} thay vì trung bình: tại mỗi vùng không gian, ưu tiên lấy chi tiết từ modality nào cung cấp thông tin sắc nét hơn.
\end{itemize}

Từ phân tích đó, CDDFuse-AG áp dụng \textbf{hai quy tắc tổng hợp khác nhau} cho hai thành phần: quy tắc trung bình có trọng số học được (WAvg) cho nhánh Base, và quy tắc lựa chọn theo độ sắc nét cục bộ (SML --- Sum-Modified-Laplacian) cho nhánh Detail. Toàn bộ kiến trúc Encoder--Decoder của CDDFuse được giữ nguyên; phần thay đổi duy nhất là cơ chế kết hợp đặc trưng tại Fusion Layer.
...
\begin{itemize}\itemsep4pt
    \item \textbf{Đề xuất mô hình CDDFuse-AG} với thiết kế bất đối xứng: Weighted Average Scalar (WAvg) cho nhánh Base và Sum-Modified-Laplacian (SML) cho nhánh Detail. Mô hình chỉ bổ sung $4{,}161$ tham số so với CDDFuse ($\approx 0{,}35\%$), nhưng cải thiện rõ rệt các chỉ số đo chất lượng chi tiết: \textbf{SF, Qabf, AG, EI, QM, QMI} --- đây là các thang đo phản ánh trực tiếp mức độ bảo toàn cạnh, tần số không gian và thông tin chi tiết mà bác sĩ cần trong chẩn đoán.

    \item \textbf{Khảo sát thực nghiệm hệ thống} theo chiến lược ablation ba giai đoạn kiểm soát một yếu tố tại một thời điểm, bao gồm 13 quy tắc tổng hợp (5 quy tắc Base, 8 quy tắc Detail), cung cấp bằng chứng thực nghiệm toàn diện về vai trò của thiết kế quy tắc tổng hợp bất đối xứng trong MIF.
\end{itemize}
...
\begin{itemize}\itemsep4pt
    ...
    \item \textbf{Chương~\ref{ch:discussion} --- Thảo luận.} Phân tích và lý giải các phát hiện thực nghiệm, bao gồm hiện tượng Modality-Specificity, vai trò của thiết kế bất đối xứng và tính hiệu quả tham số, cũng như các hạn chế còn tồn tại của đồ án.
