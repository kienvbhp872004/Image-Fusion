...
\begin{frame}{Ablation Stage 1 \& Stage 2}
    \begin{columns}[T]
        \begin{column}{0.47\textwidth}
            \small\textbf{Stage 1 --- Base rule} (Detail = Sum):
            \vspace{0.2em}
            ...
            \end{tabular}}
            \vspace{0.2em}
            \small $\Rightarrow$ \textbf{WAvg winner} (\#1 CT-MRI).
        \end{column}
        \begin{column}{0.47\textwidth}
            \small\textbf{Stage 2 --- Detail rule} (Base = Sum):
            \vspace{0.2em}
            \resizebox{\columnwidth}{!}{\footnotesize
            \begin{tabular}{lcccc}
                \toprule
            ...
            \end{tabular}}
            \vspace{0.2em}
            \small $\Rightarrow$ \textbf{SML winner} (ổn định nhất 3 modality).
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Ablation Stage 3 --- Chọn CDDFuse-AG}
    \textbf{Stage 3 --- Kết hợp bất đối xứng} (Base = WAvg):
    \vspace{0.3em}
    \small
    \begin{tabular}{lcccc}
        \toprule
    ...
    \begin{columns}[T]
        \begin{column}{0.52\textwidth}
            \small\textbf{Tại sao chọn Comb-WAvg-SML?}
            \begin{itemize}
                \item Trên 6 chỉ số paper: thắng \textbf{10/18} (L1Norm: 8/18)
                \item Ổn định hơn trên PET; \textbf{\#1 mean rank} toàn ablation
            \end{itemize}
        \end{column}
        \begin{column}{0.44\textwidth}
            \begin{alertblock}{\small Kết luận}
                \small \textbf{Sym-AG} yếu nhất ($-0.189$) --- xác nhận \textbf{bất đối xứng} hiệu quả hơn đối xứng.
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[T]
        \begin{column}{0.54\textwidth}
            ...
            \begin{enumerate}
                ...
                \item \textbf{Modality-Specificity}: CT-MRI hưởng lợi nhiều nhất
        ...
        \begin{column}{0.42\textwidth}
            \begin{block}{Kết quả nổi bật}
                \begin{itemize}
                    ...
                    \item CT z-score: $+0.309$ (CDDFuse: $-0.014$)
                    \item z avg \textbf{+0.344} --- cao nhất nhóm
                \end{itemize}
            \end{block}
            \vspace{0.3em}
            \begin{alertblock}{Hạn chế}
                \small SML nhạy artifact màu giả PET; chưa reader study lâm sàng.
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}
